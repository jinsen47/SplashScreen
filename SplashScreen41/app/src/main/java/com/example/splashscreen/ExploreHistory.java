...
public class ExploreHistory extends Activity{
	...
	String login_id;
	String passwordmd5;
    private static final String[] explore_mode={"选择户号","选择表号"}; 
    ...
    private Handler mhandler=new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			SimpleAdapter madapter=new MySimpleAdapter(ExploreHistory.this, exlist, 
			...
			case MyConfig.MSG_METER:
				Toast.makeText(ExploreHistory.this, "查询表历史读数记录成功！", 
			...
			case MyConfig.MSG_MAINTENANCE:
				Toast.makeText(ExploreHistory.this, "查询表历史维修记录成功！", 
						Toast.LENGTH_SHORT).show();
				...
				madapter=new MySimpleAdapter(ExploreHistory.this, exlist, 
			...
			case MyConfig.MSG_FAILED:
				Toast.makeText(ExploreHistory.this, "暂无历史记录", 
						Toast.LENGTH_SHORT).show();
				if(exlist!=null){
					exlist.clear();
					madapter.notifyDataSetChanged();
			...
			case MyConfig.MSG_ERROR:
				Toast.makeText(ExploreHistory.this, "网络错误", 
						Toast.LENGTH_SHORT).show();
				if(exlist!=null){
					exlist.clear();
					madapter.notifyDataSetChanged();
		...
		try{
			...
			if(cursor.moveToFirst()){//这里moveToFirst指该cursor的第一行
				do{
					...
					Log.i("History_getCons_no:", cons_no);
				}while(cursor.moveToNext());
			}
			if(cursor!=null)
			    cursor.close();
	...
	void initView(){
		...
		progressDialog=new ProgressDialog(ExploreHistory.this);
   ...
   OnItemSelectedListener itemSelectedListener2=new Spinner.OnItemSelectedListener() {
		...
		public void onItemSelected(AdapterView<?> parent, View v, int position,
				long id) {
					...
					Log.i("beginDatetemp->",beginDatetemp);
					Log.i("endDatetemp->",endDatetemp);
		}
		@Override
		public void onNothingSelected(AdapterView<?> arg0) {	
		}
